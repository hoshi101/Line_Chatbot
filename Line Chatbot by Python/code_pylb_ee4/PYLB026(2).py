...
def handle_text_message(event):
    ...
    if text == 'กลุ่มดาวฤกษ์มีอะไรบ้าง':
        ...
        template_message = TemplateSendMessage(alt_text='Hello', template=carousel_template)
        line_bot_api.reply_message(event.reply_token, template_message)
        
    elif text == 'รายละเอียดของ กลุ่มดาวนายพราน':
        text_out = ("กลุ่มดาวนายพราน ประกอบด้วยดวงดาว 3 ดวง เรียงต่อกันเป็นรูปเข็มขัดของนายพราน "
                    "และดวงอื่นๆ ที่ลากเส้นต่อกันแล้วเหมือนรูปคันธนู มีแสงสว่างปานกลาง "
                    "ผู้ที่อยู่ซีกโลกเหนือสังเกตเห็นได้ในช่วงเย็นของเดือนตุลาคม ถึง มกราคม "
                    "คนไทยรู้จักในชื่อกลุ่มดาวเต่า ดาวดวงที่สว่างที่สุดคือ “ดาวไรเจล”")
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=text_out))

    elif text == 'รูปกลุ่มดาวนายพราน':
        url = request.url_root + 'static/orion.png'
        line_bot_api.reply_message(event.reply_token, ImageSendMessage(original_content_url=url, preview_image_url=url))

    elif text == 'รายละเอียดของ กลุ่มดาวลูกไก่':
    ...
    elif text == 'รูปกลุ่มดาวลูกไก่':
        url = request.url_root + 'static/Pleiades.jpg'
        line_bot_api.reply_message(event.reply_token, ImageSendMessage(original_content_url=url, preview_image_url=url))
        
    elif text == 'รายละเอียดของ กลุ่มดาวหมีเล็ก':
    ...
    elif text == 'รูปกลุ่มดาวหมีเล็ก':
        url = request.url_root + 'static/ursa minor.jpg'
        line_bot_api.reply_message(event.reply_token, ImageSendMessage(original_content_url=url, preview_image_url=url))

    elif text == 'รายละเอียดของ ดาวหมีใหญ่':
    ...
    elif text == 'รูปกลุ่มดาวหมีใหญ่':
        url = request.url_root + 'static/ursa major.jpg'
        line_bot_api.reply_message(event.reply_token, ImageSendMessage(original_content_url=url, preview_image_url=url))
